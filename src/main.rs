...
async fn main(spawner: Spawner) {
    ...
    let system = tasks::system::System::new(board.power_hold_gpio, board.watchdog);

    spawner.spawn(tasks::system::task(system)).unwrap();
    spawner.spawn(tasks::ui::task(ui)).unwrap();
}
